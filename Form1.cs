...
namespace FileSplitter
{
    ...
    */
	public partial class Form1 : Form
	{
		...
		/*! Enumeration that stores the path in assembly to Template.cs */
		...
		/*public string ByteArrayToString(byte[] ba)
{
   ...
   return hex.ToString();
		...
		private void Form1_Load(object sender, EventArgs e)
		{
			if (!Directory.Exists(cdir + @"\\maps"))
			{
				var directoryInfo = Directory.CreateDirectory(cdir + @"\\maps");
		...
		private byte[] getChunk(string filepath)
		{
			FileInfo fi = new FileInfo(filepath);
			var ramc = new System.Diagnostics.PerformanceCounter("Mono Memory", "Total Physical Memory");
			if (fi.Length < ramc.RawValue)
			{
				return File.ReadAllBytes(filepath);
			}
			long iterations = fi.Length - ramc.RawValue;
			Byte[] fbytes = new Byte[iterations];
			using (FileStream fs = File.OpenRead(filepath))
			{
				if (lastpos != 0L && lastpos != -1) 
				{
					for (long i = lastpos; i < iterations; i++) 
					{
						fbytes[i] = (byte)fs.ReadByte();
						lastpos = fs.Position;
					}
				}
